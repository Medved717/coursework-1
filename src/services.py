...
def get_analysis_increased_cashback(
    transactions: list[dict], month: str, year: str
) -> list[dict]:
    ...
    logger.debug("Объединена дата в функции get_analysis_increased_cashback")
    ...
    for transaction in transactions:
        ...
        if (
            transaction_strp.strftime("%m.%Y") == input_date.strftime("%m.%Y")
            and transaction["Кэшбэк"] != ""
            and "-" not in str(transaction["Кэшбэк"])
        ):
            logger.debug("Пройдено условие по Кэшбэку")
            list_cashback_tranansactions.append(transaction)
            logger.info("Получен список словарей со списком транзакций с кэшбэком.")
...
def get_categories_cashback(transactions):
    ...
    for transaction in transactions:
        if transaction.get("Категория") and transaction.get("Кэшбэк") is None:
            ...
            logger.debug(
                "Пройдено условие по категориям и кэшбэку в функции get_categories_cashback."
            )
...
logger.debug(
    "Получен json - файл с транзакциями  кэшбеком в функции get_categories_cashback."
)
