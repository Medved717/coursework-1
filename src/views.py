...
def get_time():
    ...
    logger.info("Получена дата в функции get_time.")
    return data_str
...
def get_transactions_excel():
    ...
    logger.info(
        "Получен файл транзакций в формате excel и переведен в список словарей в функции \n"
...
def mask_card(transactions: list[dict]) -> list[dict]:
    ...
    logger.info(
        "Получена маскировка карт в списке словарей с транзакциями в функции mask_card."
    )
...
def cashback(transactions: list[dict]) -> list[dict]:
    ...
    logger.info("Посчитан кэшбек в списке транзакций в функции cashback.")
    return transactions
...
def get_result_list_transaction_by_date(
    ...
    datetime_beginning_of_the_month = datetime_input.replace(
        day=1, hour=0, minute=0, second=0
    )
    logger.debug(
        "Получена начальния дата в функции get_result_list_transaction_by_date."
    )

    # Формируем условие, при котором транзации формируются исходя из периода дат.
    list_transactions = []
    for transaction in transactions:
        transaction_obj_date = datetime.strptime(
    ...
    logger.info(
        "Получен список словарей с начала месяца по полученной дате в формате от меньшей даты к большей\n"
        "в функции get_result_list_transaction_by_date."
    )
...
def payment_amount(transactions: list[dict]) -> list[dict]:
    ...
    sorted_transaction = sorted(
        transactions, key=lambda x: x.get("Сумма платежа"), reverse=True
    )
...
def get_stocks():
    ...
    logger.info(
        "Получен csv - файл со списком реализуемых акций и их стоимостями в функции get_stocks."
    )
    return response
...
def get_csv_stocks() -> list[dict]:
    ...
    result_list_stocks = [
        {"stock": x["symbol"], "price": x["price"]}
        for x in file_to_dict
        if x["symbol"] in ["AAPL", "AMZN", "GOOGL", "MSFT", "TSLA"]
    ]
    logger.info(
        "Преобразован файл csv в объект пайтон (словарь) и выведены необходимые словари \n"
        " с наименованием компании и стоимостью акции. в функции get_csv_stocks"
    )
...
def file_csv_stocks():
    ...
    with open(path_file_csv, "w", encoding="utf-8") as f:
        ...
        logger.info("Данные по акциям сохранены (file_csv_stocks).")
    return None
...
def get_exchange_rate():
    ...
    try:
        ...
        file_path_json_stocks = os.path.dirname(
            os.path.dirname(os.path.abspath(__file__))
        )
        file_json_path = os.path.join(
            file_path_json_stocks, "data", "exchange_rate.json"
        )
        ...
        logger.info(
            "Осуществлен Прием данных по курсу валют формата json, осуществлен \n"
            'вывод словаря с данными о курсе валют "EUR" и "USD". в функции get_exchange_rate.'
        )
        return result
